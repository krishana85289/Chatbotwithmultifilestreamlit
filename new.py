...
def extract_text_from_text_document(uploaded_file):
    try:
        # Read and extract text content
        text = uploaded_file.read().decode('utf-8')  # Decode the bytes to string
        return text
...
def get_text_chunksd(text):
    ...
    return chunks
def load_csv_and_create_index(file_path):
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("<hr>", unsafe_allow_html=True)
        text_docs = st.file_uploader("Upload your Text Files and Click on the Submit & Process Button", accept_multiple_files=True)
        if st.button("Submit & Process Text file "):
            with st.spinner("Processing..."):
                raw_text = ""
                for text_doc in text_docs:
                    raw_text += extract_text_from_text_document(text_doc) + '\n'
                text_chunks = get_text_chunks(raw_text)
                get_vector_store(text_chunks)
                st.success("Done")
        ...
        if st.button(" Process CSV Document"):
            with st.spinner("Processing..."):
                file_path = save_uploaded_file(uploaded_file)
                load_csv_and_create_index(file_path)
